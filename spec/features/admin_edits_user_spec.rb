...
feature 'admin edits user', %{
  ...
  scenario 'admin edits a user without a company successfully' do
  ...
  scenario 'admin edits a user with a company successfully' do
    admin = FactoryGirl.create(:user, role: 'admin')
    company = FactoryGirl.create(:company, user_id: admin.id)
    CompanyUser.create(company_id: company.id, user_id: admin.id)
    sign_in_as(admin)
  ...
  scenario 'admin tries to edit a user from another company' do
    admin = FactoryGirl.create(:user, role: 'admin')
    ...
    CompanyUser.create(company_id: company2.id, user_id: user.id)
